...
bool PolymorphicBuiltinSpecializedOverloadInfo::init(
    SILModule &mod, BuiltinValueKind builtinKind,
  ...
  if (oldOperandTypes[0].isAddress()) {
    if (oldResultType != mod.Types.getEmptyTupleType())
      return false;
  ...
  for (SILType ty : oldOperandTypes) {
    // If after specialization, we do not have a builtin type, bail.
...
bool PolymorphicBuiltinSpecializedOverloadInfo::init(BuiltinInst *bi) {
  ...
  return init(bi->getModule(), *kind, oldOperandTypes, oldResultType);
}
...
swift::getStaticOverloadForSpecializedPolymorphicBuiltin(BuiltinInst *bi) {
  ...
  // values if needed at +1 and storing the result into an out parameter if
  // needed.
  ...
  for (auto arg : rawArgs) {
    ...
    SILValue load = builder.emitLoadValueOperation(
        bi->getLoc(), arg, LoadOwnershipQualifier::Take);
  ...
  if (info.hasOutParam) {
    builder.emitStoreValueOperation(newBI->getLoc(), newBI->getResult(0),
                                    result, StoreOwnershipQualifier::Init);
  }
